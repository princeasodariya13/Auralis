        ...
        <div className="admin-page p-6">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative">
                        ...
                        <input
                            ...
                            className="full-width pl-10 pr-4 py-2 border border-slate-300 rounded focus:outline-none focus:border-indigo-500"
                        />
                    </div>
            ...
            <div className="bg-white border border-slate-200 shadow-sm rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="full-width text-left border-collapse">
                        <thead>
                            <tr className="bg-surface-alt border-b border-slate-200">
                                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider">Ticket / Customer</th>
                                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider">Subject</th>
                                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider">Priority</th>
                                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider">Status</th>
                                <th className="p-4 text-xs font-semibold text-muted uppercase tracking-wider">Last Activity</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {loading ? (
                                ...
                                tickets.map(ticket => (
                                    <tr key={ticket._id} className="hover:bg-surface-alt transition-colors">
                                        <td className="p-4">
                                            <div className="font-medium text-slate-900">{ticket.ticketNumber}</div>
                                            <div className="text-sm text-muted">{ticket.userId?.email || 'Unknown User'}</div>
                                            <div className="text-xs text-muted mt-1">{ticket.category}</div>
                                        </td>
                                        <td className="p-4">
                ...
                {/* Pagination */}
                {pagination.pages > 1 && (
                    <div className="bg-surface-alt border-t border-slate-200 px-4 py-3 flex items-center justify-between sm:px-6">
                                        ...
                                        <button
                                            ...
                                            className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                                                pagination.page === i + 1
                                                    ...
                                                    : 'bg-white border-slate-300 text-muted hover:bg-surface-alt'
                                            }`}
                                        >
                                            {i + 1}
                                        </button>
