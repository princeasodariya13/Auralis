...
export const cartService = {
    async getCart() {
        const response = await fetch(`${API_URL}/cart`, getFetchOptions('GET'));
    ...
    async addToCart(productId, quantity = 1) {
        const response = await fetch(`${API_URL}/cart/items`, getFetchOptions('POST', { productId, quantity }));
        const json = await response.json();
    ...
    async updateQuantity(productId, quantity) {
        const response = await fetch(`${API_URL}/cart/items/${productId}`, getFetchOptions('PATCH', { quantity }));
        const json = await response.json();
    ...
    async removeFromCart(productId) {
        const response = await fetch(`${API_URL}/cart/items/${productId}`, getFetchOptions('DELETE'));
        const json = await response.json();
    ...
    async clearCart() {
        const response = await fetch(`${API_URL}/cart`, getFetchOptions('DELETE'));
        const json = await response.json();
    ...
    async mergeCart(guestItems) {
        ...
        const response = await fetch(`${API_URL}/cart/merge`, getFetchOptions('POST', { guestItems }));
        const json = await response.json();
